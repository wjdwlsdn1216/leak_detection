...
>   + 분석결과 llevel과 lrate이 상관관계가 높다는걸 heatmap을 통해 알 수 있다.
...
> ### 데이터
> - 누수 감지 주파수 범위 변수를 보면 특정 Hz를 제외하곤 거의 진동이 발생하지 않는걸 보면 향후 누수 감지 예측시 필요없는 주파수는 굳이 확인하지 않고 특정 주파수를 더욱 면밀히 살피면 좋을것 같다.
> - 최대 누수 크기 변수를 보면 정상음 제외 나머지는 수치가 거의 비슷하기 때문에 정확한 예측을 위해서는 클래스별 데이터 개수를 어느정도 맞춰야 더욱 높은 정확도를 도출 할 것 같다.  
> - 특정 지역의 누수 확률이 높은 경우도 있을것이다, 그러므로 지역 변수를 추가한다면 왜 이 지역의 누수가 자주 발생하는지에 대한 부분까지 알 수 있을꺼같다.
> ### 모델 평가
> - 누수 감지 모델은 재현율(recall)이 더 중요 지표이다, 이유는 실제 누수 발생상황에 Negative로 판단할 경우 누수를 방치하게 되기 때문이다.
> - 모델 학습 결과를 보면 정밀도(precision) 수치가 재현율(recall)보다 대부분 높은걸 볼 수 있는데 이러한 결과는 데이터의 4가지 소음 진동 수치 범위가 비슷하기 때문이고, 또한 모델 구축시 알고리즘과 하이퍼파라미터를 수정하다보면 더욱 높일 수 있을것이다.
